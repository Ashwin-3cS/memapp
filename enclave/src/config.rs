...
// Provider hostnames and token-endpoint paths are deliberately NOT here.
// This config is read from the host-supplied environment, so anything in it
// is host-controlled; an operator who could repoint the token endpoint could
// harvest the client secret. See services/http.rs::Upstream.
