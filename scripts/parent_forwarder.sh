...
pkill -f "VSOCK-LISTEN:8002" || true
pkill -f "VSOCK-LISTEN:8003" || true
pkill -f "VSOCK-LISTEN:8004" || true
pkill -f "VSOCK-LISTEN:8005" || true
pkill -f "VSOCK-LISTEN:5000" || true
sudo pkill -f "socat.*VSOCK:.*:4000" || true
sleep 1

echo "Starting parent forwarder..."

echo "Forwarding VSOCK 8002 -> Google OAuth APIs..."
socat VSOCK-LISTEN:8002,fork,reuseaddr TCP:www.googleapis.com:443 &

echo "Forwarding VSOCK 8003 -> GitHub API..."
